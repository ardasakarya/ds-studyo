...
export interface ThemeToggleProps
  ...
  /** İkonun yanında görünen etiket (butonun ne yaptığı belli olsun diye). */
  label?: ReactNode;
}
...
function ActionSwapIcon({ value, className, children }: ActionSwapIconProps) {
  ...
  return (
    <motion.span
      key={value}
      initial={
        reduce ? { opacity: 1 } : { opacity: 0, scale: 0.85, filter: "blur(8px)" }
      }
      animate={
  ...
  const toggle = () => {
    ...
    const vt = (
      ...
      /* next-themes özniteliği bir effect'te yazıyor; geçişin yeni kareyi
         doğru yakalaması için DOM'u burada senkron güncelliyoruz. */
...
export function ThemeToggle({
  ...
  return (
    <button
      type="button"
      aria-label={
        mounted && isDark ? "Aydınlık temaya geç" : "Koyu temaya geç"
      }
